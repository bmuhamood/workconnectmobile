...
export default function RegisterWorkerScreen() {
  ...
  const handleRegister = async () => {
    if (!form.first_name || !form.last_name || !form.email || !form.phone || !form.password || !form.profession) {
  ...
  return (
    <Screen style={{ backgroundColor: '#1e1b4b' }}>
      <AuthHero title="Join as a Worker" subtitle="Find real work across Uganda" onBack={() => router.back()} />
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: '#f9fafb', borderTopLeftRadius: 24, borderTopRightRadius: 24, marginTop: -24 }}>
        <ScrollView contentContainerStyle={{ padding: spacing.xl, paddingTop: spacing.xl + 4 }}>
          ...
          <Input label="Profession *" icon="briefcase-outline" value={form.profession} onChangeText={(v) => update('profession', v)} placeholder="e.g. Nanny, Driver, Gardener" />
